/**
 * Default ontology dataset for the Context-Aware Workplace Modeler.
 *
 * All IDs use kebab-case with typed prefixes:
 *   wf-  = workflow
 *   sys- = system
 *   usr- = persona/user
 *
 * getDefaultData() returns a fresh deep copy on every call — callers may
 * mutate the result freely without affecting the canonical constants below.
 */
...
const DEFAULT_PERSONAS = [
  { id: 'usr-firefighter',  name: 'Reactive Firefighter',  state: 'reactive-firefighter', description: 'High stress, rapid switching.' },
  { id: 'usr-deep-focus',   name: 'Deep Focus Architect',  state: 'deep-focus-architect',  description: 'Prolonged silence needed.' },
  { id: 'usr-process-admin',name: 'Process Admin',          state: 'process-admin',         description: 'Methodical queue clearing.' },
  { id: 'usr-bridge-builder',name: 'Bridge Builder',        state: 'bridge-builder',        description: 'Connecting teams.' },
]
...
const DEFAULT_FRICTION_RULES = {
  'wf-mgmt-escalations::sys-sharepoint':    0.85,
  'wf-mgmt-escalations::sys-slack-teams':    0.3,
  'wf-mgmt-escalations::sys-ai-engine':      0.2,
  'wf-mgmt-escalations::sys-jira':           0.6,
  'wf-mgmt-escalations::sys-exec-dashboard': 0.4,
  'wf-admin-deadlines::sys-sharepoint':      0.3,
  'wf-admin-deadlines::sys-slack-teams':      0.5,
  'wf-admin-deadlines::sys-ai-engine':        0.4,
  'wf-admin-deadlines::sys-jira':             0.15,
  'wf-admin-deadlines::sys-exec-dashboard':   0.5,
  'wf-system-maintenance::sys-sharepoint':    0.4,
  'wf-system-maintenance::sys-slack-teams':    0.55,
  'wf-system-maintenance::sys-ai-engine':      0.35,
  'wf-system-maintenance::sys-jira':           0.2,
  'wf-system-maintenance::sys-exec-dashboard': 0.65,
  'wf-cross-team-sync::sys-sharepoint':       0.5,
  'wf-cross-team-sync::sys-slack-teams':       0.2,
  'wf-cross-team-sync::sys-ai-engine':         0.45,
  'wf-cross-team-sync::sys-jira':              0.3,
  'wf-cross-team-sync::sys-exec-dashboard':    0.55,
  'wf-strategic-planning::sys-sharepoint':     0.5,
  'wf-strategic-planning::sys-slack-teams':     0.9,
  'wf-strategic-planning::sys-ai-engine':       0.15,
  'wf-strategic-planning::sys-jira':            0.7,
  'wf-strategic-planning::sys-exec-dashboard':  0.25,
}
...
/**
 * Returns a fresh deep copy of the canonical ontology dataset.
 * Safe to mutate; mutations will not affect subsequent calls.
 */
export function getDefaultData() {
  return {
    workflows: DEFAULT_WORKFLOWS.map(w => ({ ...w, linkedSystems: [...w.linkedSystems] })),
    systems: DEFAULT_SYSTEMS.map(s => ({
      ...s,
      linkedWorkflows: [...s.linkedWorkflows],
      linkedUsers: [...s.linkedUsers],
    })),
    personas: DEFAULT_PERSONAS.map(p => ({ ...p })),
    contextMap: JSON.parse(JSON.stringify(DEFAULT_CONTEXT_MAP)),
    frictionRules: { ...DEFAULT_FRICTION_RULES },
    modeRules: JSON.parse(JSON.stringify(DEFAULT_MODE_RULES)),
  }
}
